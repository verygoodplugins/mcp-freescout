...
`mcp-freescout` is an MCP (Model Context Protocol) server that MCP server for FreeScout ticket management and workflow automation.
...
- Register tools with `McpServer.registerTool` and Zod schemas.
